...
async function loadBackendVersion() { if (!api) { backendVersion.textContent = "גרסת שרת: לא הוגדרה"; return; } try { const response = await fetch(apiUrl("/health")); const data = await response.json(); if (!response.ok || !data.version) throw new Error(); backendVersion.textContent = `גרסת שרת: ${data.version}`; } catch { backendVersion.textContent = "גרסת שרת: לא זמינה"; } }
...
async function copyExportPath(path, button) { try { await navigator.clipboard.writeText(path); } catch { const input = document.createElement("textarea"); input.value = path; input.style.position = "fixed"; input.style.opacity = "0"; document.body.append(input); input.select(); document.execCommand("copy"); input.remove(); } const previous = button.textContent; button.textContent = "הועתק"; window.setTimeout(() => { button.textContent = previous; }, 1400); }
function exportPaths(exportName) { const base = [dataRoot?.name, "clients", committedWorkspace?.directory?.name, "declarations", currentDeclaration?.month, "exports", exportName].filter(Boolean).join("\\"); return { pdf: `${base}\\invoices.pdf`, txt: `${base}\\import.txt` }; }
function showExportResult({ title, message, exportName = "", error = false }) { const dialog = document.querySelector("#export-result-dialog"), heading = document.querySelector("#export-result-title"), body = document.querySelector("#export-result-message"), paths = document.querySelector("#export-result-paths"); heading.textContent = title; body.textContent = message; body.className = error ? "export-result-error" : "export-result-success"; paths.hidden = error; if (!error) { const values = exportPaths(exportName), pdf = document.querySelector("#export-pdf-path"), txt = document.querySelector("#export-txt-path"), copyPdf = document.querySelector("#copy-export-pdf-path"), copyTxt = document.querySelector("#copy-export-txt-path"); pdf.textContent = values.pdf; txt.textContent = values.txt; copyPdf.onclick = () => copyExportPath(values.pdf, copyPdf); copyTxt.onclick = () => copyExportPath(values.txt, copyTxt); } dialog.showModal(); }
async function validateExport(snapshots) { const templateText = new TextDecoder("windows-1255").decode(await (await canonicalTemplate.getFile()).arrayBuffer()), issues = validateRivhitImport({ templateText, rows: snapshots, mapping: rivhitMapping, declarationMonth: currentDeclaration?.month }); if (issues.length) { showExportValidation(issues); return null; } return templateText; }
...
async function activateDeclaration(selected) {
  ...
  refreshRows(); setTableLocked(currentDeclaration.status !== "open"); currentClient.textContent = `לקוח: ${workspace.config.clientName} · הצהרה: ${currentDeclaration.month}`; journalTitle.textContent = currentClient.textContent; applyBusinessRules(); status.textContent = currentDeclaration.status === "open" ? "" : "ההצהרה סגורה לקריאה בלבד."; updateStartAvailability(); setWorkspacesDrawer(false); return true;
}
...
function clearActiveClient(clientId) { if (workspace?.config.clientId === clientId) workspace = null; if (committedWorkspace?.config.clientId === clientId) { committedWorkspace = null; currentDeclaration = null; currentClient.textContent = "לא נבחרה הצהרה"; journalTitle.textContent = "לקוח והצהרה לא נבחרו"; updateStartAvailability(); } }
function clearActiveDeclaration(declarationId) { if (currentDeclaration?.declarationId !== declarationId) return; currentDeclaration = null; currentDeclarationDirectory = null; records.replaceChildren(emptyRow); recordCount = 0; currentClient.textContent = "לא נבחרה הצהרה"; journalTitle.textContent = "לקוח והצהרה לא נבחרו"; updateStartAvailability(); }
async function switchDataRoot(selected) {
  await saveCurrentDraft();
  dataRoot = selected; dataRootTitle.textContent = `תיקיית נתונים: ${selected.name}`; workspace = null; committedWorkspace = null; currentDeclaration = null; currentDeclarationDirectory = null; canonicalTemplate = null; recordCount = 0; imageCount = 0; received = new Set(); newCustomCodes = new Set();
  records.replaceChildren(emptyRow); currentClient.textContent = "לא נבחרה הצהרה"; journalTitle.textContent = "לקוח והצהרה לא נבחרו"; workspaceSummary.textContent = "נבחרה תיקיית נתונים חדשה. יש לבחור לקוח והצהרה."; workspaceSummary.classList.remove("workspace-ready");
...
createPdf.addEventListener("click", async () => {
  ...
  try {
    ...
    status.textContent = `ייצוא טיוטה נשמר: ${name}`; showExportResult({ title: "הייצוא הושלם", message: "קובצי PDF ו-TXT נשמרו בהצלחה.", exportName: name });
  } catch (error) { if (error.name !== "AbortError") { const message = `לא ניתן ליצור ייצוא טיוטה: ${error.message}`; showError(message); showExportResult({ title: "הייצוא נכשל", message, error: true }); } }
...
closeDeclarationButton.addEventListener("click", async () => {
  ...
  try {
    ...
    await Promise.all([writeFile(directory, "invoices.pdf", pdf), writeFile(directory, "classification-codes.pdf", classificationsPdf), writeFile(directory, "import.txt", importText), writeFile(directory, "manifest.json", JSON.stringify(manifest, null, 2))]);
    currentDeclaration = await finalizeDeclaration({ clientDirectory: committedWorkspace.directory, declarationDirectory: currentDeclarationDirectory, declaration: currentDeclaration, finalExport: name, rows: snapshots }); setTableLocked(true); updateStartAvailability(); status.textContent = `ההצהרה נסגרה. הייצוא הסופי נשמר: ${name}`; showExportResult({ title: "ההצהרה נסגרה והייצוא הושלם", message: "קובצי PDF ו-TXT הסופיים נשמרו בהצלחה.", exportName: name });
  } catch (error) { if (error.name !== "AbortError") { const message = `לא ניתן לסגור את ההצהרה: ${error.message}`; showError(message); showExportResult({ title: "הייצוא נכשל", message, error: true }); } updateStartAvailability(); }
});
