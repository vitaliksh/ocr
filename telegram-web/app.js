...
records.addEventListener("change", (event) => { const row = event.target.closest?.("tr[data-document-id]"), cell = event.target.closest?.("td"), column = row && cell ? [...row.cells].indexOf(cell) : -1; if (row && column === 6) updateDuplicateState(row); queueDraftSave(); });
