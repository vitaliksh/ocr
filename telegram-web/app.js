...
async function loadCustomMapping(root) { customMapping = await readCustomRivhitMapping(root, builtInMapping); rivhitMapping = { ...builtInMapping, ...customMapping }; refreshClassificationSelectors(); }
const workspaceControls = setupWorkspaceControls({
  ...
  onDataRoot: (selected) => { dataRoot = selected; loadCustomMapping(selected).catch((error) => showError("לא ניתן לטעון קודי מיון מקומיים: " + error.message)); updateStartAvailability(); },
...
function applyRecord(row, record) {
  ...
  row.cells[2].replaceChildren(classificationSelect(record.rivhit_code || "")); row.cells[11].replaceChildren(percentSelect(record.vat_recognized_percent ?? 100, [100, 25, 0], "vat")); row.cells[12].replaceChildren(percentSelect(record.recognized_percent || 100, [100, 25, 0], "expense")); applyBusinessRule(row);
...
function restoreRow(saved, blob) {
  ...
  row.cells[2].replaceChildren(classificationSelect(values[1] || "")); row.cells[11].replaceChildren(percentSelect(values[10] || 100, [100, 25, 0], "vat")); row.cells[12].replaceChildren(percentSelect(values[11] || 100, [100, 25, 0], "expense")); recalculateRow(row);
...
saveCustomClassification.addEventListener("click", async () => {
  ...
  try {
    saveCustomClassification.disabled = true; customMapping = await saveCustomRivhitMapping(dataRoot, { ...customMapping, [code]: label }, builtInMapping); rivhitMapping = { ...builtInMapping, ...customMapping };
...
document.querySelector("#popout-photo").addEventListener("click", () => {
  if (!dialogImage.src) return; const popup = window.open("", "rivhit-document-image", "popup=yes,width=1000,height=800");
  if (!popup) return showError("הדפדפן חסם את החלון הנפרד. יש לאפשר חלונות קופצים לאתר זה.");
  popup.document.title = photoTitle.textContent || "תמונת מסמך"; popup.document.body.style.cssText = "margin:0;background:#111;display:grid;place-items:center";
  const image = popup.document.createElement("img"); image.src = dialogImage.src; image.alt = photoTitle.textContent || "תמונת מסמך"; image.style.cssText = "max-width:100vw;max-height:100vh;object-fit:contain"; popup.document.body.append(image); popup.focus();
});
