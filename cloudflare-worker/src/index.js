...
async function recognizeWithGemini(request, env) {
  ...
  try { const parsed = JSON.parse(data.candidates?.[0]?.content?.parts?.[0]?.text || ""), records = parsed.records.map((record) => normalizeRecord(record, mapping)), hasExpense = records.some((record) => record.document_kind === "expense_invoice"); return json({ records: records.filter((record) => !(hasExpense && record.document_kind === "other" && record.total_amount === null && !record.supplier_name && !record.date)) }); }
