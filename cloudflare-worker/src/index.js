...
}, required: ["document_kind", "confidence", "agent_opinion", "date", "supplier_name", "supplier_vat_id", "invoice_number", "transaction_number", "allocation_number", "purpose", "total_amount", "currency", "form_6111_code", "recognized_percent", "vat_recognized_percent", "net_amount", "vat_amount", "vat_percent", "document_number_box", "total_amount_box", "vat_amount_box"] } } }, required: ["records"] };
const HIGHLIGHT_SCHEMA = { type: "OBJECT", properties: { records: { type: "ARRAY", items: { type: "OBJECT", properties: { index: { type: "INTEGER" }, document_number_box: { type: "ARRAY", nullable: true, minItems: 4, maxItems: 4, items: { type: "NUMBER" } }, total_amount_box: { type: "ARRAY", nullable: true, minItems: 4, maxItems: 4, items: { type: "NUMBER" } }, vat_amount_box: { type: "ARRAY", nullable: true, minItems: 4, maxItems: 4, items: { type: "NUMBER" } } }, required: ["index", "document_number_box", "total_amount_box", "vat_amount_box"] } } }, required: ["records"] };
...
async function fillMissingHighlights({ records, image, contentType, selectedModel, env }) {
  const missing = records.map((record, index) => ({ record, index })).filter(({ record }) => !record.highlight_regions?.length);
  if (!missing.length) return records;
  const targets = missing.map(({ record, index }) => ({ index, document_number: record.transaction_number || record.invoice_number, total_amount: record.total_amount, vat_amount: record.vat_amount }));
  const prompt = `Locate printed values on this Israeli financial document image. Return boxes only for these already-extracted records: ${JSON.stringify(targets)}. For every visible value, return a tight [ymin, xmin, ymax, xmax] box normalized 0–1000. document_number_box must surround the printed document or transaction number; total_amount_box surrounds the printed gross total; vat_amount_box surrounds the printed VAT amount. Do not include labels, whitespace, table cells, or a different value. A box may be null only when that exact value is not visibly printed or is calculated. Never change the records or infer a value.`;
  try {
    const payload = JSON.stringify({ system_instruction: { parts: [{ text: prompt }] }, contents: [{ role: "user", parts: [{ text: "Find the requested printed value locations." }, { inline_data: { mime_type: contentType, data: base64Encode(image) } }] }], generationConfig: { response_mime_type: "application/json", response_schema: HIGHLIGHT_SCHEMA, temperature: 0 } });
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(selectedModel)}:generateContent`, { method: "POST", headers: { "content-type": "application/json", "x-goog-api-key": env.GEMINI_API_KEY }, body: payload });
    const data = await response.json(); if (!response.ok) throw new Error();
    for (const item of JSON.parse(data.candidates?.[0]?.content?.parts?.[0]?.text || "").records || []) if (Number.isInteger(item.index) && records[item.index] && !records[item.index].highlight_regions?.length) records[item.index].highlight_regions = highlights(item);
  } catch { console.error("Gemini highlight fallback failed"); }
  return records;
}
...
let promptMapping = RIVHIT_MAPPING;
function mappingPrompt(mapping = promptMapping) { return Object.entries(mapping).map(([code, [rivhit, name]]) => `- Form 6111 ${code} → Rivhit ${rivhit}: ${name}`).join("\n"); }
function normalizeRecord(raw, mapping = RIVHIT_MAPPING) {
  const kind = ["expense_invoice", "payment_confirmation", "income_report", "other"].includes(raw?.document_kind) ? raw.document_kind : "other";
  const expense = kind === "expense_invoice", income = kind === "income_report", code = text(raw?.form_6111_code), mapped = expense && code ? mapping[code] : null;
...
async function recognizeWithGemini(request, env) {
  ...
  try { const parsed = JSON.parse(data.candidates?.[0]?.content?.parts?.[0]?.text || ""), records = parsed.records.map((record) => normalizeRecord(record, mapping)); await fillMissingHighlights({ records, image, contentType, selectedModel, env }); const hasExpense = records.some((record) => record.document_kind === "expense_invoice"); return json({ records: records.filter((record) => !(hasExpense && record.document_kind === "other" && record.total_amount === null && !record.supplier_name && !record.date)) }); }
...
async function refineWithHistory(request, env) {
  ...
  const selectedModel = request.headers.get("x-gemini-model") || env.GEMINI_MODEL || "gemini-3.5-flash-lite", mapping = form6111Mapping(request); promptMapping = mapping;
  if (!GEMINI_MODELS.has(selectedModel)) return json({ error: "Unsupported Gemini model." }, 400);
  ...
  try { const raw = JSON.parse(data.candidates?.[0]?.content?.parts?.[0]?.text || ""), code = text(raw.form_6111_code), mapped = code ? mapping[code] : null; return json({ rivhit_code: mapped?.[0] || null, recognized_percent: percentage(raw.recognized_percent), vat_recognized_percent: percentage(raw.vat_recognized_percent), confidence: percentage(raw.confidence), review_state: raw.review_state === "ready" ? "ready" : "review", agent_opinion: text(raw.agent_opinion) || "לא נמסר הסבר מהסוכן." }); } catch { return json({ error: "Gemini returned an invalid refinement." }, 502); }
}
